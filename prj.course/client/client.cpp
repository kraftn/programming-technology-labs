...
void Client::DownloadSettings() {
  ...
  if (!path.isNull()) {
    ...
    int32_t count_point(0);
    for (ptrdiff_t i_string(0); i_string < IP_address_.length(); i_string += 1) {
      if ('.' == IP_address_[i_string]) {
        count_point += 1;
      }
    }
    if (count_point != 3) {
    ...
    try {
      ...
      std::string cut_IP_address(IP_address_);
      int32_t part(0);
      for (int32_t i(0); i < 3; i += 1) {
        ...
        cut_IP_address.erase(0, cut_IP_address.find_first_of('.') + 1);
      }
