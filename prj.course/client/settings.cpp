  ...
  for (ptrdiff_t i_interfaces(0); i_interfaces < interfaces.size();
    ...
    if (interfaces[i_interfaces].flags().testFlag(QNetworkInterface::IsRunning)) {
      ...
      for (ptrdiff_t i_entries(0); i_entries < entries.size(); i_entries += 1) {
        if (QAbstractSocket::IPv4Protocol == entries[i_entries].ip().protocol()
            && !entries[i_entries].ip().isLoopback()) {
          quint32 network(0);
          network = entries[i_entries].ip().toIPv4Address() &
                    entries[i_entries].netmask().toIPv4Address();
          networks_.push_back(network);
          netmasks_.push_back(entries[i_entries].netmask().toIPv4Address());
          ui.comboBox_networks->addItem(QHostAddress(network).toString() + " / " +
...
void Settings::Scan() {
  ...
  for (ptrdiff_t i(1); i < pow(2, amount) - 1; i += 1) {
    network += 1;
    std::string endpoint("tcp://" + QHostAddress(network).toString().toStdString() +
                         ":60000");
